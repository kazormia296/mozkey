...
def adjust_cost(entry: Entry, profile: str, stats: Stats) -> Entry:
    ...
    if profile == "daily":
        ...
        # Do not delete useful entries only because the reading is short.
        # Instead, demote them so common daily conversions are less likely to be displaced.
        cost_floor = 7800
        ...
        # Recall-oriented, but still avoids giving UT entries too much priority.
        cost_floor = 7300
        ...
        # Almost raw. Only hard rejects and duplicate removal are applied.
        pass

    else:
        raise ValueError(f"unknown profile: {profile}")

    # Keep well inside int16-ish practical bounds used by many dictionary pipelines.
...
def load_entries(path: pathlib.Path, profile: str) -> tuple[list[Entry], Stats]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        for line in f:
            ...
            signature = (entry.key, entry.lid, entry.rid, entry.value)
