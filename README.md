...
- ライブ変換を使わない場合も、既定で Space 1回目の通常変換で第1候補のまま候補ウィンドウを開ける（設定で無効化可能）
...
### ライブ変換 OFF 時の Space 初回候補表示

ライブ変換を使わない場合の補助機能として、Space キー 1 回目の通常変換で候補ウィンドウを開く設定を追加しています。この設定は既定で有効です。

この設定が有効な場合、未確定文字列の入力中に Space を 1 回押した時点で通常変換に入り、第1文節の候補ウィンドウを表示します。このとき選択候補は第1候補のままで、第2候補へは進みません。さらに Space を押した場合は、従来どおり次候補へ進みます。

この機能はライブ変換がオフの場合だけ適用されます。ライブ変換がオンの場合は、既存のライブ変換中の Space 操作、つまり通常変換候補への移行と候補移動の挙動を優先します。従来の表示タイミングに戻したい場合は、設定画面から無効化できます。
