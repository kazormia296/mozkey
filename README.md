...
<p align="center">
  ...
  <img alt="macOS status" src="https://img.shields.io/badge/macOS-untested-lightgrey">
</p>
...
本 fork は、主に個人の Windows 利用と実験的な Linux Fcitx5 連携のために、
Mozc に入力補助・ライブ変換・文脈補正・ローカル Zenz 補正・オフライン配布向けの
調整を加えたものです。
...
- Zenz 補正は `mozc_server` から Windows named pipe / Linux Unix socket 経由で scorer に依頼し、platform の `llama-server` localhost endpoint でローカル推論
...
- Zenz model / llama.cpp runtime の third-party license notice を Windows / Linux 製品に同梱
...
この fork では、llama.cpp ベースのローカル Zenz 推論 runtime を使用します。Windows は runtime を同梱し、Linux は検証済み distro `llama-server` を product-private link から起動します。どちらもローカル推論用 server として `127.0.0.1` のみに bind し、random port と API key を使います。外部ネットワークサービスを公開したり、入力内容を外部サーバーへ送信したりする目的のものではありません。
...
`mozc_zenz_scorer.exe` に送り、同梱の `llama-server.exe` を使います。Linux では
Unix socket 経由で `mozc_zenz_scorer` に送り、product-private link から検証済みの
distro `llama-server` を起動します。どちらも random port、API key、
...
This fork is mainly maintained for personal Windows use and an experimental
Linux Fcitx5 integration. It adds input assistance, live conversion,
context-aware conversion, local Zenz correction, and
offline-distribution-oriented adjustments to Mozc.
...
This fork uses a local Zenz inference runtime based on llama.cpp. Windows
bundles the runtime; Linux launches a verified distribution `llama-server`
through a product-private link. Both bind only to `127.0.0.1` on a random port
and require an API key. They are not intended to expose an external network
service or send user input to external servers.
...
- Sends Zenz correction requests through a Windows named pipe or Linux Unix
  socket. Windows uses the bundled `llama-server.exe`; Linux launches a
  verified distribution `llama-server` through a product-private link.
...
- Bundles third-party license notices for the Zenz model and llama.cpp runtime in Windows and Linux products
...
named pipe and the scorer launches the bundled `llama-server.exe`. On Linux,
the request uses a Unix socket to `mozc_zenz_scorer`, which launches a verified
distribution `llama-server` through a product-private link. Both use a random
port, an API key, and a `127.0.0.1`-only endpoint for local inference. The
transport rejects accidental or stale local endpoint mismatches.
